...
public class EditItem extends BaseClass implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        if (v == testDueDate){
            ...
            String[] dateSplitTestDueDate = testDueDate.getText().toString().split("-");
            if(dateSplitTestDueDate.length > 1){
                fromDatePickerDialog.updateDate(Integer.valueOf(dateSplitTestDueDate[0]), Integer.valueOf(dateSplitTestDueDate[1]) - 1, Integer.valueOf(dateSplitTestDueDate[2]));
            }
            else{
                String[] dateNow = Core.getDateNowStringWithoutTime().split("-");
                fromDatePickerDialog.updateDate(Integer.valueOf(dateNow[0]), Integer.valueOf(dateNow[1]) - 1, Integer.valueOf(dateNow[2]));
            }

        }
        else if(v == endOfLiveDate){
            selectedDate = "endOfLiveDate";
            String[] dateSplitEndOfDate = endOfLiveDate.getText().toString().split("-");
            if(dateSplitEndOfDate.length > 1){
                fromDatePickerDialog.updateDate(Integer.valueOf(dateSplitEndOfDate[0]), Integer.valueOf(dateSplitEndOfDate[1]) - 1, Integer.valueOf(dateSplitEndOfDate[2]));
            }
            else{
                String[] dateNow = Core.getDateNowStringWithoutTime().split("-");
                fromDatePickerDialog.updateDate(Integer.valueOf(dateNow[0]), Integer.valueOf(dateNow[1]) - 1, Integer.valueOf(dateNow[2]));
